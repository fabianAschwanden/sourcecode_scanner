...
| WR-03a | S | Die gleichzeitig laufenden Scans SOLLEN serverseitig begrenzt werden (`scanner.scan.max-concurrent`, OOM-Schutz). Übersteigt der Start das Limit, wird der Scan **eingereiht** (Status `QUEUED`) und nach Freiwerden eines Slots automatisch gestartet; die UI zeigt den `QUEUED`-Status und erlaubt das Abbrechen eingereihter Läufe. Beim Neustart verwaiste `RUNNING`/`QUEUED`-Läufe werden auf `FAILED` gesetzt. |
| WR-03b | S | Der Scan-Modus SOLL wählbar sein: **nur Default-Branch ohne History** (`HEAD`, flacher Single-Branch-Clone — schnell, RAM-arm; Default) oder **mit History** (`FULL`, alle Branches). Die History wird beim Lauf **commit-für-commit** gestreamt (Slicing), sodass der Peak-Speicher ein Commit ist, nicht das ganze Repo. |
...
| WR-66a | S | Die `<Datei>:<Zeile>`-Angabe einer Ergebniszeile SOLL ein **Deep-Link zur Quelle** sein: aus der Repo-Location (Git-URL, GitHub/GitLab/Bitbucket inkl. SSH-/`.git`-Form) + Pfad + Zeile wird ein Web-Permalink gebaut (`blob/<branch>/<pfad>#L<zeile>`, Bitbucket `src/<branch>/<pfad>#lines-<zeile>`); Branch = erster konfigurierter Branch, sonst `HEAD`. Der Link öffnet in neuem Tab (`rel=noopener`). Ist die Quelle unbekannt oder kein Web-Host erkennbar (z. B. lokales Repo), bleibt es reiner Text ohne Link. Es wird nie Klartext übertragen (WR-68). |
