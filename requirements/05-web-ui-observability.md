...
| WR-31a | S | Als IdP-Variante KANN GitHub (OAuth2) genutzt werden (Profil `ghauth`, `quarkus.oidc.provider=github`). Da GitHub keine Rollen liefert, leitet ein Augmentor die Rolle aus dem Login ab: Allowlist (`scanner.auth.github.admin-logins`) → `admin`; sonst öffentliches Mitglied der konfigurierten Org (`scanner.auth.github.org`, geprüft über `GET /orgs/{org}/members/{login}`) → `operator`; sonst → `viewer`. |
